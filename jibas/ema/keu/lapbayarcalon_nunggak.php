...
<tr>
    ...
    <tr>
        ...
        <?  $sql = "SELECT replid, nama FROM $db_name_fina.datapenerimaan WHERE aktif = 1 AND idkategori = '$idkategori' ORDER BY replid DESC";
            $result = QueryDb($sql);
            while ($row = mysql_fetch_array($result)) {
                if ($idpenerimaan == 0) 
	...
	if ($_REQUEST[kat]=="jtt")
	{
		...
		?>
		 ...
		 <?		}
			...
			<? }?>
				<td align="center">
		<?	$sql = "SELECT max(datediff('$tgl', tanggal)) FROM $db_name_fina.penerimaanjttcalon WHERE idbesarjttcalon = '$idbesarjtt'";
			$result2 = QueryDb($sql);
			$row2 = mysql_fetch_row($result2);
			echo $row2[0]; ?>
				</td>
