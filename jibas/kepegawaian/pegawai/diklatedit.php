...
if (isset($_REQUEST['btSubmit']))
{
	...
	$sql = "UPDATE pegdiklat SET iddiklat=$iddiklat, tahun=$thndiklat, sk='$sk', keterangan='$keterangan', doaudit = 1 WHERE replid = $id";
	QueryDb($sql);
	CloseDb(); ?>
...
<?
	...
	OpenDb();
	$sql = "SELECT p.iddiklat, d.diklat, p.tahun, p.sk, p.keterangan FROM pegdiklat p, diklat d WHERE p.replid = $id AND p.iddiklat = d.replid";	
	$result = QueryDb($sql);
