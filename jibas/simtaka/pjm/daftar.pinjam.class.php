        ...
        <tr>
            ...
            <td width="91%">
				<select name="kriteria" id="kriteria" onchange="chgKrit()">
					<option value="all" <?=StringIsSelected('all',$this->kriteria)?> >100 Peminjaman Terakhir</option>
...
<?	  	if ($num > 0)
		{
			...
			while ($row = @mysql_fetch_array($result))
			{
				...
				if ($this->jenisanggota == "siswa")
					$this->idanggota = $row['nis'];
				elseif ($this->jenisanggota == "pegawai")
					$this->idanggota = $row['nip'];
				else
					$this->idanggota = $row['idmember'];
