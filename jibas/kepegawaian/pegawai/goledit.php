...
{
	OpenDb();
	$sql = "UPDATE peggol SET golongan='$gol', tmt='$tmt', sk='$sk', keterangan='$keterangan', doaudit = 1 WHERE replid=$id";
	QueryDb($sql);
	CloseDb(); ?>
...
{
	OpenDb();
	$sql = "SELECT golongan, tmt, sk, keterangan FROM peggol WHERE replid=$id";
	$result = QueryDb($sql);
