...
if (isset($_REQUEST['btSubmit']))
{
	...
	if ($success)
	{
		$sql = "UPDATE jbssdm.peggol SET terakhir=0 WHERE nip='$nip'";
		QueryDbTrans($sql, $success);	
	}
	
	if ($success)
	{
		$sql = "INSERT INTO jbssdm.peggol
					  SET nip='$nip', golongan='$gol', tmt='$tmt', sk='$sk', keterangan='".CQ($keterangan)."', terakhir=1";
		QueryDbTrans($sql, $success);	
	}
	...
	if ($success)
	{
		$sql = "UPDATE jbssdm.peglastdata SET idpeggol=$idpeggol WHERE nip='$nip'";
		QueryDbTrans($sql, $success);	
	}
