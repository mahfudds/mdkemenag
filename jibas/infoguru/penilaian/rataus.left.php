...
<table width="100%" border="1" cellspacing="0" cellpadding="0" class="tab" id="table"
       style="border-collapse: collapse; border-width: 1px; border-color: #f5f5f5;">
    <tr style="height: 2px;">
        <td></td>
    </tr>
<?
...
while ($row = @mysqli_fetch_row($res)){
    ...
    <tr style="height: 24px;">
    	<td class="td" style="cursor:pointer" onclick="parent.right.location.href='rataus.right.php?pel=<?=$row[0]?>&nis=<?=$nis?>&kls=<?=$kls?>&tkt=<?=$tkt?>'"><a target="right" href="rataus.right.php?pel=<?=$row[0]?>&nis=<?=$nis?>&kls=<?=$kls?>&tkt=<?=$tkt?>"><?=$row[3]?></a></td>
    </tr>
    <?
}
