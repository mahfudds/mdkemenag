...
function tampil2(tanggal,bulan,tahun) {
 // alert(tanggal);
	parent.kiribawah.location.hash = "tgl" + tanggal;
}
</script>
</head>
...
for ($i = 0; $i < count($cal); $i++) { 
	...
	for ($j = 0; $j < count($cal[$i]); $j++) {
		...
		if (($bln == $bulan) && ($thn == $tahun)){
			$style = "thismonth";
			$a =  "<a title='Klik untuk melihat agenda tanggal ".$tgl."-".$bln."-".$thn."' style=\"text-decoration:none;\" href=\"JavaScript:tampil2('".$tgl."','".$bln."','".$thn."')\"> <font class='$style'>$tgl</font></a>";
		} else {
			$style = "othermonth";
			$a =  "<font class='$style'>$tgl</font>";
		}
		if ($j == 0)
			$color = "#FFCCCC";
		elseif ($j == 5) 
