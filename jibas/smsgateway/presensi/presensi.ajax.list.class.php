...
class PresensiListAjax{
	...
	function DeleteInfoGenList(){
		$sql = "DELETE FROM outboxhistory WHERE idsmsgeninfo='$this->IdInfoGen'";
		QueryDb($sql);
		$sql = "DELETE FROM outbox WHERE idsmsgeninfo='$this->IdInfoGen'";
		QueryDb($sql);
	...
	function GetInfoGenList(){
		...
		<table width="300" class="tab" border="1" cellspacing="0" cellpadding="0" id="Table1">
		  ...
		  <?
		  $sql = "SELECT replid,info FROM smsgeninfo WHERE tipe=0 AND YEAR(tanggal)='".$this->Thn."' AND MONTH(tanggal)='".$this->Bln."' ORDER BY replid DESC";
		  //echo $sql;
		  $res = QueryDb($sql);
