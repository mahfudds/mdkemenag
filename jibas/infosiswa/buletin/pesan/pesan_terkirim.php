...
<table width="100%" border="0" cellspacing="0">
  ...
  */
  $sql_tot="SELECT * FROM jbsvcr.pesanterkirim pt, jbsvcr.pesan p WHERE p.nis='".SI_USER_ID()."' AND pt.idpesan=p.replid";
  //echo $sql_tot;
  ...
  $sql1="SELECT pt.replid as replid_tkrm, pg.replid as replid, pg.judul as judul, DATE_FORMAT(pg.tanggalpesan, '%e %b %Y') as tanggal, TIME_FORMAT(pg.tanggalpesan, '%H:%i') as waktu FROM jbsvcr.pesan pg, jbsvcr.pesanterkirim pt WHERE pg.nis='".SI_USER_ID()."' AND pt.idpesan=pg.replid ORDER BY pg.tanggalpesan DESC, pg.replid DESC LIMIT ".(int)$page*(int)$varbaris.",$varbaris";
