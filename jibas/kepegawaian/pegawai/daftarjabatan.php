...
<tr>
	<td width="100%" align="left" style="border-bottom:thin dashed #CCCCCC; border-top:none; border-left:none; border-right:none;">
        ...
        <font class="subtitle">Jadwal Kenaikan Jabatan</font><br />
    </td>
</tr>
<tr><td>
...
<?
$sql = "SELECT replid, DATE_FORMAT(tanggal,'%d %M %Y') AS ftmt, keterangan FROM jadwal WHERE nip='$DJ->nip' AND jenis='jabatan'";
$result = QueryDb($sql);
...
<?	} // while 
	...
	<tr height="80">
    	<td colspan="4" align="center" valign="middle">
            ...
            <font color="#999999"><strong>Belum ada jadwal kenaikan jabatan pegawai ini.<br />Klik <input type="button" name="btTambah" value="di sini" class="but" style="color:#003399" onclick="JavaScript:TambahJ()" /> untuk mengisi jadwal kenaikan jabatan</strong></font>
...
<?
$sql = "SELECT p.replid, p.jenis, DATE_FORMAT(p.tmt,'%d %M %Y') AS ftmt, p.sk, p.keterangan, p.terakhir, p.namajab FROM pegjab p WHERE p.nip = '$DJ->nip' ORDER BY tmt DESC";
$result = QueryDb($sql);
$cnt = 0;
while ($row = mysqli_fetch_array($result)) {
?>
