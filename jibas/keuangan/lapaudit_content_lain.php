...
function cetak() 
{
	var addr = "lapaudit_lain_cetak.php?idtahunbuku=<?=$idtahunbuku?>&departemen=<?=$departemen?>&tanggal1=<?=$tanggal1?>&tanggal2=<?=$tanggal2?>";
	newWindow(addr, 'CetakAuditLain','780','580','resizable=1,scrollbars=1,status=0,toolbar=0');
}

function excel() 
{
	var addr = "lapaudit_lain_excel.php?idtahunbuku=<?=$idtahunbuku?>&departemen=<?=$departemen?>&tanggal1=<?=$tanggal1?>&tanggal2=<?=$tanggal2?>";
	newWindow(addr, 'ExcelAuditLain','780','580','resizable=1,scrollbars=1,status=0,toolbar=0');
}
</script>
</head>
  	...
  	<table width="100%" border="0" height="100%" cellspacing="0" cellpadding="0">
   	<tr>
		<td><font size="2" color="#990000">
        	<strong>Perubahan Data Penerimaan Lain</strong></font>
        </td>
        <td align="right">
