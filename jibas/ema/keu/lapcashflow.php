...
OpenDb();
$sql = "SELECT day(now()), month(now()), year(now()), day(date_sub(now(), INTERVAL 1 MONTH)), month(date_sub(now(), INTERVAL 1 MONTH)), year(date_sub(now(), INTERVAL 1 MONTH))";
$result = QueryDb($sql);
...
function change_dep() {
	...
	//parent.content.location.href = "lapcashflow_blank.php";
	var addr = "lapcashflow.php?idtahunbuku="+idtahunbuku+"&tgl1="+tgl1+"&bln1="+bln1+"&thn1="+thn1+"&tgl2="+tgl2+"&bln2="+bln2+"&thn2="+thn2+"&departemen="+dep;
...
function show_laporan() {
	...
	if (validasi)	
		sendRequestText("get_lapcashflow.php", showLap, "departemen="+dep+"&tanggal1="+tanggal1+"&tanggal2="+tanggal2+"&idtahunbuku="+idtahunbuku+"&bln="+bln2+"&thn="+thn2);	
...
function cetak() {
	...
	var addr = "lapcashflow_cetak.php?departemen="+dep+"&tanggal1="+tanggal1+"&tanggal2="+tanggal2+"&idtahunbuku="+idtahunbuku+"&bln="+bln2+"&thn="+thn2;
	newWindow(addr, 'CetakArusKas','790','630','resizable=1,scrollbars=1,status=0,toolbar=0');	
}
function showLap(x){
