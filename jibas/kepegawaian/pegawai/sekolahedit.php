...
{
	OpenDb();
	$sql = "UPDATE pegsekolah SET tingkat='$tingkat', sekolah='$sekolah', lulus=$lulus, sk='$sk', keterangan='$keterangan', doaudit = 1 WHERE replid = $id";
	QueryDb($sql);
	CloseDb(); ?>
...
{
	OpenDb();
	$sql = "SELECT tingkat, sekolah, lulus, sk, keterangan FROM pegsekolah WHERE replid = $id";	
	$result = QueryDb($sql);
