...
function ShowCbKegiatan($nip, $idkegiatan)
{
    echo "<select id='cbKegiatan' onchange='changeOption()'>\r\n";
    
    $sql = "SELECT DISTINCT p.idkegiatan, k.kegiatan
    ...
    {
        $sel = $row[0] == $idkegiatan ? "selected" : "";
        echo "<option value='" . $row[0] . "' $sel>" . $row[1] . "</option>";
    }
    $sel = -1 == $idkegiatan ? "selected" : "";
    echo "<option value='-1' $sel>(Semua Kegiatan)</option>/r/n";
    echo "</select>\r\n";
}
?> 