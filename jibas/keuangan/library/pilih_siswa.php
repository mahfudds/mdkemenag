...
?>
	<tr height="25" onClick="pilih('<?=$row[0]?>')" style="cursor:pointer" id="siswapilih<?=$cnt?>">
		...
		<td align="center">
		<input type="text" name="pilihsiswa<?=$cnt?>" id="pilihsiswa<?=$cnt?>" readonly="readonly" size="10" style="border:none; background:none; text-align:center;" value="<?=$row[0]?>" onkeypress="pilih('<?=$row[0]?>');return focusNext1('siswa', event, 'pilih', <?=$cnt?>, 1)" />
