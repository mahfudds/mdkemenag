...
function getPilihanPelajaran($viewDaftarUjian)
{
    try
    {
        ...
        $request->ViewDaftarUjian = $viewDaftarUjian;

        $http = new HttpManager();
        $http->setData("pelajaran", CbeState::PilihanUjianRemedial, "0", $jsonSession, $request->toJson());
        $result = $http->send();
...
function getDaftarUjian($viewDaftarUjian, $idPelajaran)
{
    try
    {
        ...
        $http = new HttpManager();
        $http->setData("daftar", CbeState::DaftarUjianRemedial, "0", $jsonSession, $info->toJson());
        $result = $http->send();
...
function createTableUjian($jsonUjian)
{
    ...
    for ($i = 0; $i < count($ujianArr); $i++)
    {
        ...
        $table .= "onclick='ur_startUjian($no)'>";
