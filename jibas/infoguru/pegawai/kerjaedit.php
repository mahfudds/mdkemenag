...
{
	OpenDb();
	$sql = "UPDATE jbssdm.pegkerja
               SET tempat='$tempat', jabatan='$jabatan', thnawal='$thnawal',
                   thnakhir='$thnakhir', keterangan='$keterangan'
             WHERE replid = $id";
	QueryDb($sql);
	CloseDb(); ?>
...
{
	OpenDb();
	$sql = "SELECT thnawal, thnakhir, tempat, jabatan, keterangan
              FROM jbssdm.pegkerja
             WHERE replid = $id";	
	$result = QueryDb($sql);
