...
if (isset($_REQUEST['btSubmit']))
{
	...
	if ($success)
	{
		$sql = "UPDATE pegdiklat SET terakhir=0 WHERE nip='$nip'";
		QueryDbTrans($sql, $success);	
	}
	
	if ($success)
	{
		$sql = "INSERT INTO pegdiklat SET nip='$nip', iddiklat=$iddiklat, tahun=$thndiklat, sk='$sk', terakhir='1', keterangan='$keterangan'";
		QueryDbTrans($sql, $success);	
	}
	...
	if ($success)
	{
		$sql = "UPDATE peglastdata SET idpegdiklat=$idpegdiklat WHERE nip='$nip'";
		QueryDbTrans($sql, $success);	
	}
