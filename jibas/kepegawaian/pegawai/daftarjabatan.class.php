...
class DaftarJabatan
{
    ...
    public function __construct()
    {
        ...
        elseif ($op == "cn0948cm2478923c98237n23") 
        {
            ...
            $sql = "UPDATE pegjab SET terakhir=0 WHERE nip='$this->nip'";
            QueryDbTrans($sql, $success);
            
            if ($success)
            {
                $sql = "UPDATE pegjab SET terakhir=1 WHERE replid=$id";
                QueryDbTrans($sql, $success);
            }
            
            if ($success)
            {
                $sql = "UPDATE peglastdata SET idpegjab=$id WHERE nip='$this->nip'";
                QueryDbTrans($sql, $success);
            }
        ...
        elseif ($op == "mnrmd2re2dj2mx2x2x3d2s33")
        {
            ...
            $sql = "DELETE FROM pegjab WHERE replid=$id";	
            QueryDbTrans($sql, $success);
            
            if ($success)
            {
                $sql = "UPDATE peglastdata SET idpegjab=NULL WHERE idpegjab=$id AND nip='$this->nip'";
                QueryDbTrans($sql, $success);
            }
