...
<?
$sql = "SELECT replid, DATE_FORMAT(tanggal,'%d %M %Y') AS ftmt, keterangan FROM jadwal WHERE nip='$nip' AND jenis='jabatan'";
$result = QueryDb($sql);
...
<?
$sql = "SELECT p.replid, p.jenis, DATE_FORMAT(p.tmt,'%d %M %Y') AS ftmt, p.sk, p.keterangan, p.terakhir, p.namajab FROM pegjab p
