...
<div>
	<?
	$sql = "SELECT replid,semester FROM semester WHERE departemen='$dep'";
	$res = QueryDb($sql);
	while ($row = @mysql_fetch_row($res)){
		...
		<? for ($i=0;$i<count($semester);$i++)
		   {
			...
			<div class="TabbedPanelsContent">
				...
				<div id="TabbedPanelsA<?=$i?>" class="TabbedPanels" style="z-index:1">
				  <ul class="TabbedPanelsTabGroup">
<? 					$sql5 = "SELECT DISTINCT a.dasarpenilaian, d.keterangan
					...
					<div class="TabbedPanelsContent">
						...
						while ($row = @mysql_fetch_row($res)){
							...
							while ($row2 = @mysql_fetch_row($res2)){
								...
								$ratauk = $row[0];
								$prosen = round((($row2[2]  - $row3[0]) / $row2[2]) * 100, 2);
								if ($prosen>0)
									$prosen = "+".$prosen;	
								$numnilai += $row2[2];
