...
if (isset($_REQUEST['btSubmit']))
{
	...
	if ($success)
	{
		$sql = "UPDATE jbssdm.pegsekolah SET terakhir=0 WHERE nip='$nip'";
		QueryDbTrans($sql, $success);	
	}
	
	if ($success)
	{
		$sql = "INSERT INTO jbssdm.pegsekolah
					  SET nip='$nip', tingkat='$tingkat', sekolah='".CQ($sekolah)."', lulus=$lulus, sk='$sk',
					      terakhir='1', keterangan='".CQ($keterangan)."'";
		QueryDbTrans($sql, $success);	
	}
	...
	if ($success)
	{
		$sql = "UPDATE jbssdm.peglastdata SET idpegsekolah=$idpegsekolah WHERE nip='$nip'";
		QueryDbTrans($sql, $success);	
	}
