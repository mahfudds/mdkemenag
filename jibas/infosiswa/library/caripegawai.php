...
if (isset($_REQUEST['Submit'])) { 
	...
	if ((strlen($nama) > 0) && (strlen($nip) > 0))
		$sql = "SELECT nip, nama FROM jbssdm.pegawai WHERE nama LIKE '%$nama%' AND nip LIKE '%$nip%' AND nip NOT IN (SELECT login FROM jbsuser.hakakses WHERE MODUL='INFOGURU') ORDER BY nama"; 
	else if (strlen($nama) > 0)
		$sql = "SELECT nip, nama FROM jbssdm.pegawai WHERE nama LIKE '%$nama%' AND nip NOT IN (SELECT login FROM jbsuser.hakakses WHERE MODUL='INFOGURU')ORDER BY nama"; 
	else if (strlen($nip) > 0)
		$sql = "SELECT nip, nama FROM jbssdm.pegawai WHERE nip LIKE '%$nip%' AND nip NOT IN (SELECT login FROM jbsuser.hakakses WHERE MODUL='INFOGURU')ORDER BY nama"; 
	else if ((strlen($nama) == 0) || (strlen($nip) == 0)) 
		$sql = "SELECT nip, nama FROM jbssdm.pegawai WHERE nip NOT IN (SELECT login FROM jbsuser.hakakses WHERE MODUL='INFOGURU')ORDER BY nama";		
} else {
	$sql = "SELECT nip, nama FROM jbssdm.pegawai WHERE nip NOT IN (SELECT login FROM jbsuser.hakakses WHERE MODUL='INFOGURU')ORDER BY nama"; 
}
$result = QueryDb($sql);	
