...
OpenDb();
$sql = "SELECT p.departemen FROM kelompokcalonsiswa k, prosespenerimaansiswa p WHERE k.replid='$kelompok' AND k.idproses=p.replid ";
$result = QueryDb($sql);
...
<title>JIBAS EMA [Laporan Tunggakan Iuran Sukarela Calon Siswa Per Kelas]</title>
...
<?
OpenDb();
$sql = "SELECT c.replid, c.nopendaftaran, c.nama, k.kelompok FROM calonsiswa c, kelompokcalonsiswa k WHERE c.idkelompok = k.replid AND c.replid IN ($nisstr) "; 
$result = QueryDb($sql);
if ($page==0)
	$cnt = 0;
else 
