...
class DaftarGolongan
{
    ...
    public function __construct()
    {
        ...
        elseif ($op == "cn0948cm2478923c98237n23") 
        {
            ...
            $sql = "UPDATE peggol SET terakhir=0 WHERE nip='$this->nip'";
            QueryDbTrans($sql, $success);
            
            if ($success)
            {
                $sql = "UPDATE peggol SET terakhir=1 WHERE replid=$id";
                QueryDbTrans($sql, $success);
            }
            
            if ($success)
            {
                $sql = "UPDATE peglastdata SET idpeggol=$id WHERE nip='$this->nip'";
                QueryDbTrans($sql, $success);
            }
        ...
        elseif ($op == "mnrmd2re2dj2mx2x2x3d2s33")
        {
            ...
            $sql = "DELETE FROM peggol WHERE replid=$id";	
            QueryDbTrans($sql, $success);
            
            if ($success)
            {
                $sql = "UPDATE peglastdata SET idpeggol=NULL WHERE idpeggol=$id AND nip='$this->nip'";
                QueryDbTrans($sql, $success);
            }
