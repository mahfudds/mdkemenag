	...
	if ($_REQUEST[kat] == "jtt")
	{
		...
		?>
		 ...
		 <?		}
			...
			<? }?>
				<td align="center">
		<?	$sql = "SELECT datediff('$tgl', max(tanggal)) FROM jbsfina.penerimaanjtt WHERE idbesarjtt = '$idbesarjtt'";
			//echo $sql;
			$result2 = QueryDb($sql);
			$row2 = mysql_fetch_row($result2);
			echo $row2[0]; ?>
	...
	//DISINI BUAT SKR=====================================================================================================
		...
		<?	$sql = "SELECT count(*) FROM $db_name_fina.penerimaaniuran WHERE nis = '$nis' AND idpenerimaan = $idpenerimaan";
		 ...
		 <?		}
			...
			<? }?>
				<td align="center">
		<?	$sql = "SELECT max(datediff('$tgl', tanggal)) FROM $db_name_fina.penerimaaniuran WHERE nis = '$nis' AND idpenerimaan = '$idpenerimaan'";
			echo $sql;
			$result2 = QueryDb($sql);
			$row2 = mysql_fetch_row($result2);
			echo $row2[0]; ?>
